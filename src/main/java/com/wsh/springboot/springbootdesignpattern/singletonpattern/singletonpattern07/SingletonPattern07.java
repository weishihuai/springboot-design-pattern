 ...
 * <p>
 * 说明:
 * 这种方式跟饿汉式方式采用的机制类似，但又有不同。两者都是采用了类装载的机制来保证初始化实例时只有一个线程。不同的地方在饿汉式方式是只要Singleton类被装载就会实例化，没有Lazy-Loading的作用，而静态内部类方式在Singleton类被装载时并不会立即实例化，而是在需要实例化时，调用getInstance方法，才会装载InnerSingletonInstance类，从而完成Singleton的实例化。
 * <p>
...
class Singleton {
    ...
    /**
     * 第二步: 使用静态内部类，该类有一个静态属性Singleton对象
     * 当Singleton类被装载时，静态内部类是不会进行装载的。
     * 只有使用getInstance()方法用到INSTANCE时内部类才会被装载，起到懒加载的效果
     * 类装载是线程安全的
     */
